...
interface NativeSelectProps<V extends { label: string | JSX.Element | null; value: any }> {
	value: V | undefined;
...
export const NativeSelect: <V extends { label: string | JSX.Element | null; value: any }>(props: NativeSelectProps<V>) => JSX.Element | null = React.memo(({
	value,
	onChange,
	options,
	placeholder,
}) => {
	const selectedIndex = value ? options.findIndex(e => e.value === value.value) : -1;
	...
	return <select value={selectedIndex} placeholder={placeholder || undefined} onChange={handleChange}>
...
export const EditTextComponent = WrapInCustHookChangeError(
	React.memo(({ path, stat, usePlaceholder, onChange }: EditTextComponentProps) => {
		const haRef = useRef<any>(null);
		const Cont = useEditCustomizationProp("elements", path);
		const placeholder = usePlaceholder ? usePlaceholder.use() : usePlaceholder;
		...
		return <Cont placeholder={placeholder} ref={haRef} onChange={handleChange} defaultValue={stat.text} />;
	})
);
