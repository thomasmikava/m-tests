...
import { getAbsoluteProperty, getUpdatedAbsoluteProperty } from "../../../../utils/prop";

const useContentType = () =>
	EditContentCont.useSelector(x => x.content.type, []);
	
const useContentSetter = () =>
	EditContentCont.useSelector(x => x.onChange, []);

const useContentGetterFn = () =>
	EditContentCont.useSelector(x => x.getContent, []);

const useExplanation = () =>
	EditContentCont.useSelector(
		x => x.content.explanation,
		[]
	);
...
const useContentPathValueSetter = (path: ContentPath): SetState<IStatement> => {
	...
	return useOptimizedFunc((vl) => {
		updateContent(x => {
			const currentValue: IStatement = getAbsoluteProperty(x, path.relativePath);
